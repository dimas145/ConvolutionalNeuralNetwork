...
class Dense:
    ...
    def backward_propagation(self, chain_matrix=None, z=0, target=0):
        if z == 1:
            ...
            self._dE_do = Utils.dE_dnet(loss, target).transpose().dot(self._weights)
            self._dE_dw = Utils.dE_dnet(loss, target).dot([self._input_neurons])
            ...
            self._dE_dw = np.dot(temp.transpose(), np.array([self._input_neurons]))
